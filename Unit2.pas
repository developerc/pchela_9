...
begin
//Нажатие на кнопку "Земля"
Form1.Log.Clear;
Form1.Log.Lines.LoadFromFile('priz.txt');
Unit1.priz_value:= StrToInt(Form1.Log.Lines.Strings[0]);
ScktAddr:= Form1.Log.Lines.Strings[1];//УСТАН IP-адрес сервера
Unit1.Priority:= TrimRight(Form1.Log.Lines.Strings[2]);//устан приоритета рабоч места
...
Unit1.RemoteHost:= Form1.Log.Lines.Strings[6];
   //закомментарим подключение к серверу обмена сообшениями
 ...
 //if Form1.ClientSocket2.Open=false then ShowMessage('SMS сервер не доступен!');
 //if Form1.ClientSocket2.Active=false then ShowMessage('SMS сервер не доступен!');
