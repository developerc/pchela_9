...
var
  ...
  count: integer;
  DateTime : TDateTime;
  n : integer=1;  //задали значение порядкового номера позвонившего телефона
 ...
 // NewNumF : integer;
  ...
  Priz : integer;
  flag_priz : integer=0;
  flag_abon : integer=0; //если в таблице заказы абонент
  flag_predv : integer=0; //предварительный заказ
  ...
  ArrayStrGrd3 : array[0..100,0..100,0..1] of integer; //массив таблицы машин 
  //ret: TStringDynArray;
 // Rect: TRect;
...
//******инициализация очереди**************
 ...
 SetLength(Queue, 0);
 QueueSize:=0;
 count:=0; //счетчик пропущенных звонков
//*****************************************
 with ADOCommand1 do begin       //есть ли такой оператор
...
procedure TForm1.ClientSocket1Connecting(Sender: TObject;
  Socket: TCustomWinSocket);
begin
StatusBar1.Panels[0].Text:='Соединяюсь...';
end;

procedure TForm1.ClientSocket1Connect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
StatusBar1.Panels[0].Text:='Соединился';
...
procedure TForm1.ClientSocket1Disconnect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
StatusBar1.Panels[0].Text:='Разъединился!';
end;
...
begin  //таймер срабатывает каждую СЕКУНДУ
//если карточка регистрации заказов готова принимать входящие звонки (пустая) то
//"выталкиваем" - pop из очереди
 ...
 if (LabeledEdit1.Text='') AND (LabeledEdit2.Text='')
  ...
  then
   begin
    ...
    if s='' then
     begin
      StatusBar1.Panels[3].Text:='Очередь пуста.';
     end
    else
     begin
      SeparateString(s,';',CallingNumb,CalledNumb);
      FillCard(CallingNumb+';'+IntToStr(NumZak));  //заносим в карточку
      StatusBar1.Panels[3].Text:='Вызов с номера: '+CallingNumb+' обрабатывается';
     end;
   end
  else
   begin
    if Length(Queue)<>0 then
     begin
      StatusBar1.Panels[1].Text:='В очереди'+IntToStr(Length(Queue))+'вызова';
     end
    else
...
s:=Socket.ReceiveText;
 //ShowMessage(s);
 if (s[1]+s[2]+s[3]='CMD') then begin  //если CMD отрабатываем команду
   Delete(s,1,3);
   //Bookmark:=DBGrid1.DataSource.DataSet.Bookmark;
   //BM:=DBGrid1.DataSource.DataSet.GetBookmark;
   if (s[1]+s[2]+s[3]='REF') then begin  //если это REFRESH
    if not DBGrd1Focused then begin
     with ADOQuery1 do begin  //отображение их в таблице
     SQL.Clear;
    ...
    if (s[1]+s[2]+s[3]='ARE') then begin   //если это AREYOUREADY
    ...
    if (s[1]+s[2]+s[3]='CHD') then begin   //если это CHDISP
  //ShowMessage('Смена диспетчера');
 //////////////////////////////////////////



     with ADOCommand1 do begin       //находим запись в таблице диспетчеры
  CommandText := 'select * from disp where working=1';
  CommandType := cmdText;
 ...
 //ShowMessage('есть рабочий диспетчер');
 Working:=ADODataSet1.Recordset.Fields[0].Value;
 ShowMessage('Рабочий диспетчер '+IntToStr(Working));
 end;
///////////////////////////////////////////////
  end;
 end;
 {
 if (s[1]+s[2]+s[3]='DAT') then begin    //значит приходят данные
 Delete(s,1,3);
  if push(s) then
  begin
   StatusBar1.Panels[2].Text:='В очереди: '+IntToStr(Length(Queue))+' вызовов';
  end
 else
  begin
   StatusBar1.Panels[2].Text:='Очередь переполнена';
   Inc(Count);
   StatusBar1.Panels[1].Text:='Пропущено'+IntTostr(Count);
  end;
 end;
 }
...
StatusBar1.Panels[0].Text:='Диспетчер авторизовался'
else
ShowMessage('Авторизация неуспешная!');
...
begin
   ...
   if (AYear>0) and (AYear<9999) and (AMonth>=1) and (AMonth<=12) and (AHour>=0) and (AHour<=24) and (AMinute>=0) and (AMinute<=60) and (ASecond>=0) and (ASecond<=60) and (AMillisecond>=0) and (AMillisecond<=60) then begin
    //ничего не делаем, если время корректно
   end
   else   begin
   DateTime:=Now; //еще раз определяем время если сбой
   DecodeDate(DateTime,AYear,AMonth,ADay);
   DecodeTime(DateTime,AHour,AMinute,ASecond,AMillisecond);
   end;
 ...
 Buf_dat[n]:='';
 Buf_tim[n]:='';
 Buf_fio[n]:='';   //буфер для фамилии
 Buf_sred[n]:='';  //буфер для средней суммы

 with ADOCommand1 do begin       //есть ли такой телефон в таблице абонентов
...
ADODataSet1.Recordset.MoveFirst;  //перемещаемся по рекордсету
  ...
  if Priority = '1' then begin    //если это компьютер с 1 приоритетом
  with ADOCommand1 do begin       //добавление записей если абонент
  ...
  if Priority = '1' then begin    //если это компьютер с 1 приоритетом
  with ADOCommand1 do begin       //добавление записей если не абонент
  CommandText := 'INSERT INTO zvonki ' +
    '(TELEFON,DAT,TIM) ' +
 ...
 DBGrid2.DataSource.DataSet.Last; //перемещ указатель до конца
...
// if ADODataSet1.Recordset.Fields[2].Value <>NULL then
 ...
 Buf_tim[n]:=FormatDateTime('hh:mm',DateTime);
  if (Length(Buf_telef[n])>0) then begin //если номер определился, заполняем карточку
 LabeledEdit1.Text:='';
 LabeledEdit2.Text:='';
 LabeledEdit3.Text:='';
 LabeledEdit4.Text:='';
 ...
 LabeledEdit1.EditLabel.Caption:= CalledNumb; //в этикетку - на какой № звонят
 LabeledEdit2.Text:=Buf_kod[n];
 LabeledEdit3.Text:=Buf_bal[n];
 LabeledEdit4.Text:=Buf_adr[n];
 LabeledEdit4.EditLabel.Caption:=Buf_fio[n];//в этикетку-ФИО
...
//-------------------------------------------------------------------
  ...
  with ADOQuery2 do begin  //отображение их в таблице
SQL.Clear;
SQL.Add('select num,telefon,kode,dat,tim,adres,car,beg,en,cost,priznak,zakaz from zvonki order by num');
Open;
end;
 DBGrid2.DataSource.DataSet.Last;//перемещаем указатель в гриде до конца
 Num[n]:=DBGrid2.Fields[0].AsInteger;
...
procedure TForm1.LabeledEdit7KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin   //нажатие на поле машина
case Key of
VK_RETURN :  begin
//ShowMessage(IntToStr(NumZak));
if LabeledEdit3.Text='' then LabeledEdit3.Text:='0';
              if LabeledEdit2.Text<>'' then begin  //если позвонил абонент
 with ADOCommand1 do begin       //есть ли абонент с таким кодом в таблице заказов
...
if ADODataSet1.RecordCount>0 then begin  //ищем не цигане ли это
ADODataSet1.Recordset.MoveFirst;
if (ADODataSet1.Recordset.Fields[11].Value = 'призовая') and (StrToInt(LabeledEdit3.Text) >= priz_value) then begin
flag_zigane:=1;
ShowMessage('Этот абонент уже едет на призовой!');
end;
end;
//end;
 //----------------------
 if (NumZak>0) and (LabeledEdit7.Text='') then begin // если заказ принят автоматически и нет машины

 with ADOCommand1 do begin       //добавление записей если без машины
  ...
  Parameters.ParamByName('PPRIZNAK').Value:='призовая';
  ShowMessage('Абоненту положена призовая поездка!');
  PlaySound('sound6.wav',0,SND_FILENAME);
  end
   else  Parameters.ParamByName('PPRIZNAK').Value:='очередная';
  Parameters.ParamByName('PWORKING').Value:=Working;
  Execute;
  end;
 end;
 //----------------------
 if (NumZak=0) and (LabeledEdit7.Text='') then begin // если заказ принят вручную и нет машины
 DBGrid1.DataSource.DataSet.First;//перемещаем указатель в гриде в начало
...
NumZak:=DBGrid1.Fields[0].AsInteger-1;
 ...
 with ADOCommand1 do begin       //добавление записей если без машины
  ...
  //----------------------
 if (NumZak>0) and (LabeledEdit7.Text<>'') then begin // если заказ принят автоматически и есть машина
      ...
      if ADODataSet1.RecordCount>0 then begin //если машина в очереди
ADODataSet1.Recordset.MoveFirst;
//CurNum:=ADODataSet1.Recordset.Fields[0].Value; //находим текущий номер в таблице
//CurNum:=1;
CurCar:=ADODataSet1.Recordset.Fields[2].Value; //и текущую машину
 with ADOCommand1 do begin       //добавление записей если с машиной
  CommandText := 'INSERT INTO zakazi ' +
 ...
 // PerestavCar(CurNum,CurCar);
   with ADOCommand1 do begin
      ...
      Execute;
       ...
       ShowMessage('Такой машины нет в очереди!');
       flag_car_yes:=0;
       end;
 end;
  //----------------------
 if (NumZak=0) and (LabeledEdit7.Text<>'') then begin // если заказ принят вручную и есть машина
      with ADOCommand1 do begin
      CommandText := 'SELECT * from point_queues '+
      'WHERE CAR = :PCAR';
       CommandType := cmdText;
...
NumZak:=DBGrid1.Fields[0].AsInteger-1;
 ...
 // PerestavCar(CurNum,CurCar);
   with ADOCommand1 do begin
      ...
      //--------если позвонил не абонент----------------
                   if LabeledEdit2.Text='' then begin
           //  ShowMessage('Позвонил не абонент!');
 //----------------------
 if (NumZak>0) and (LabeledEdit7.Text='') then begin // если заказ принят автоматически и нет машины
 ...
 // Parameters.ParamByName('PKODE').Value:=LabeledEdit2.Text;
  ...
  Parameters.ParamByName('PTIME').Value:=LabeledEdit6.Text;
  Parameters.ParamByName('PADRES').Value:=LabeledEdit4.Text;
  Parameters.ParamByName('PPRIZNAK').Value:='очередная';
  Parameters.ParamByName('PWORKING').Value:=Working;
  Execute;
  end;
 end;
...
//обрабатываем VIP абонента
// ShowMessage('VipAbon = ' + IntToStr(VipAbon));
if VipAbon=1 then begin
 with ADOCommand1 do begin       //отмечаем поле VIP
  ...
  Execute;
   ...
   with ADOQuery3 do begin  //отображение очереди машин в таблице
   SQL.Clear;
...
NumZak:=0;
 ...
 HttpSndZak(LabeledEdit4.Text, LabeledEdit7.Text, IntToStr(NumZak), 'ОЧЕРЕДНАЯ', LabeledEdit1.Text);
}

  if (flag_car_yes=1) then begin
 LabeledEdit1.Text:='';
 LabeledEdit1.EditLabel.Caption:='Телефон';
 LabeledEdit2.Text:='';
 LabeledEdit3.Text:='';
 LabeledEdit4.Text:='';
 LabeledEdit4.EditLabel.Caption:='Адрес';
 LabeledEdit5.Text:='';
 LabeledEdit6.Text:='';
 LabeledEdit7.Text:='';
 //ClientSocket1.Socket.SendText(Priority+'READY');
...
procedure TForm1.LabeledEdit6KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin  // нажатие в поле время
case Key of  //нажатие на поле "время"
VK_RETURN : begin
...
procedure TForm1.LabeledEdit5KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
case Key of  //нажатие на поле "дата"
VK_RETURN : begin
...
procedure TForm1.LabeledEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
case Key of  //нажатие на поле "баланс"
VK_RETURN : begin
LabeledEdit4.SetFocus;
end;
VK_ESCAPE : begin
...
procedure TForm1.LabeledEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
case Key of  //нажатие на поле "код"
VK_RETURN : begin
 with ADOCommand1 do begin       //есть ли такой код в таблице абонентов
  CommandText := 'SELECT * FROM abonenty ' +
    'WHERE kode = :PKODE';
  CommandType := cmdText;
  Parameters.ParamByName('PKODE').Value := LabeledEdit2.Text;
...
if  ADODataSet1.Recordset.Fields[7].Value = NULL then
  ...
  begin  //заполняем поле баланс и среднее знач сумм поездок
...
begin
case Key of  //нажатие на поле "телефон"
VK_RETURN : begin
...
ClientSocket1.Socket.SendText(Priority+'READY');
 ...
 Label2.Visible:=false;
  ClientSocket3.Socket.SendText('HNG;'+IntToStr(ChanAssign));
   case ChanAssign of
   1: begin Label13.Color:=clActiveBorder; Label13.Caption:='КАНАЛ1' end;
   2: begin Label14.Color:=clActiveBorder; Label14.Caption:='КАНАЛ2' end;
   3: begin Label15.Color:=clActiveBorder; Label15.Caption:='КАНАЛ3' end;
   4: begin Label16.Color:=clActiveBorder; Label16.Caption:='КАНАЛ4' end;
   5: begin Label17.Color:=clActiveBorder; Label17.Caption:='КАНАЛ5' end;
   6: begin Label20.Color:=clActiveBorder; Label20.Caption:='КАНАЛ6' end;
   end;
...
//конец повторения нажатия ESCAPE
//повторение нажатия F1
  for i:=1 to 6 do
  if ChanActive[i] > 0 then begin
  ChanAssign:=i;
  break;
...
VK_F5 : begin
 if (LabeledEdit1.Text='') AND (LabeledEdit2.Text='')
  ...
  then
   begin
    s:=label13.Caption;
    if (s='') or (POS('КАНАЛ',s)>0) or (POS('ОТБОЙ',s)>0) then
    else
     begin
      SeparateString(s,';',CallingNumb,NmrZakLab);
      FillCard(CallingNumb+';'+NmrZakLab);  //заносим в карточку
     end;
   end;
end;
//---------------------конец нажатие F5----------------------
...
var CostPerc : real;
begin   //меню "заказ отработан"
 if DBGrid1.Fields[2].AsString <> '' then begin //если абонент
 ...
 //DBGrid1.Fields[10].AsInteger:=Round(CostPerc);
   if MessageDlg('Отработать заказ абонента на ' + FloatToStr(Round(CostPerc)) + ' руб?' ,mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
 {  DBGrid1.SetFocus;
   Dbgrid1.Fields[9].FocusControl;
 CostPerc:=DBGrid1.Fields[10].AsInteger;
 ...
 end else begin  //если не абонент
  // OtrabStoimost(NumF);
   end;
...
begin    //меню "удалить запись"
{  if MessageDlg('Удалить заказ '+ IntToStr(NumZakClick)+' ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin }
with ADOCommand1 do begin
...
begin  //нажатие на пункте меню авария
with ADOCommand1 do begin       //обновлени поля state
  ...
  with ADOQuery3 do begin  //отображение их в таблице
SQL.Clear;
SQL.Add('select * from cqueue order by num');
Open;
end;
...
begin   //нажатие на пункте меню на заказе
with ADOCommand1 do begin       //обновлени поля state
...
begin  //нажатие на пункте меню на линии
with ADOCommand1 do begin       //обновлени поля state
...
var NewCol : integer;
    ...
    LVar : OleVariant;
begin
   with ADOCommand1 do begin       //находим номер в таблице последней машины
...
StrGrdCol:=ADODataSet1.Recordset.Fields[2].Value;
   ...
   ADODataSet1.First;
   //------------------------------------------------------------------
   LVar:=ADODataSet1.Recordset.Fields[0].Value; //преобраз из OLEVariant в STRING
...
// end
// else ShowMessage('Машина не выбрана!');
ObnovStringGrig(1);
...
begin   //отметить как предварительный
 with ADOCommand1 do begin       //отмечаем поле предварительный TRUE
...
if DBGrid1.Fields[11].AsString='призовая' then DBGrid1.Canvas.Font.Color:=clGreen;
if DBGrid1.Fields[12].AsString='YES' then DBGrid1.Canvas.Font.Color:=clRed;
...
begin
    ...
    with ADOCommand1 do begin       //обновление поля dat
    ...
    with ADOCommand1 do begin       //обновление поля tim
    ...
    with ADOCommand1 do begin       //обновление поля cost
    ...
    with ADOCommand1 do begin       //обновление поля balans
    ...
    with ADOCommand1 do begin       //обновление поля poezdok
    ...
    Execute;
     ...
     if flag_priz=1 then begin
    with ADOCommand1 do begin       //обновление поля priz
   ...
   // ShowMessage('NumF= '+IntToStr(NumF));
      with ADOCommand1 do begin
  CommandText := 'select * from abonenty where kode=:PKODE'; //выбираем запись для этого абонента
  CommandType := cmdText;
  Parameters.ParamByName('PKODE').Value:=DBGrid1.Fields[2].AsString;
  ADODataSet1.Recordset := Execute;
  end;
  if ADODataSet1.Recordset.Fields[48].Value<>NULL then begin //определяем значение NumF
    ...
    //ShowMessage('NUMF=NULL');
    NumF:=1;
    end;   //определили значение NumF
  // Если призовая, удаляем все данные о предыдущих поездках и NumF:=1;
  if flag_priz=1 then begin
   NumF:=1;
 ...
 ExecProc;
  ...
  // // // // //
    case NumF of
    1 : begin   //обновляем поля first
    with ADOCommand1 do begin       //обновление поля first_d
    ...
    with ADOCommand1 do begin       //обновление поля first_t
    ...
    with ADOCommand1 do begin       //обновление поля first_c
    ...
    with ADOCommand1 do begin       //обновление поля first_p
    CommandText := 'UPDATE abonenty SET first_p=:PPRIZ ' +
    'WHERE kode=:PKODE';
    CommandType := cmdText;
    if flag_priz=1 then
    Parameters.ParamByName('PPRIZ').Value:='призовая'
    else  Parameters.ParamByName('PPRIZ').Value:='очередная';
    Parameters.ParamByName('PKODE').Value:=DBGrid1.Fields[2].AsString;
    Execute;
    end;
    with ADOCommand1 do begin       //обновление поля first_a
    ...
    with ADOCommand1 do begin       //обновление поля first_m
...
{ with ADOStoredProc4 do
 ...
 Parameters.ParamByName('cost').Value:=DBGrid1.Fields[10].AsInteger
    ...
    if flag_priz=1 then
    Parameters.ParamByName('priz').Value:='призовая'
    else  Parameters.ParamByName('priz').Value:='очередная';
 ExecProc;
 end; }
    end;
    2 : begin
    with ADOCommand1 do begin       //обновление поля second_d
    ...
    with ADOCommand1 do begin       //обновление поля second_t
    ...
    with ADOCommand1 do begin       //обновление поля second_c
    ...
    with ADOCommand1 do begin       //обновление поля second_p
    ...
    with ADOCommand1 do begin       //обновление поля second_a
    ...
    with ADOCommand1 do begin       //обновление поля second_m
    ...
    3 : begin
    with ADOCommand1 do begin       //обновление поля third_d
    ...
    with ADOCommand1 do begin       //обновление поля third_t
    ...
    with ADOCommand1 do begin       //обновление поля third_c
    ...
    with ADOCommand1 do begin       //обновление поля third_p
    ...
    with ADOCommand1 do begin       //обновление поля third_a
    ...
    with ADOCommand1 do begin       //обновление поля third_m
    ...
    4 : begin
    with ADOCommand1 do begin       //обновление поля fourth_d
    ...
    with ADOCommand1 do begin       //обновление поля fourth_t
    ...
    with ADOCommand1 do begin       //обновление поля fourth_c
    ...
    with ADOCommand1 do begin       //обновление поля fourth_p
    ...
    with ADOCommand1 do begin       //обновление поля fourth_a
    ...
    with ADOCommand1 do begin       //обновление поля fourth_m
    ...
    5 : begin
    with ADOCommand1 do begin       //обновление поля fifth_d
    ...
    with ADOCommand1 do begin       //обновление поля fifth_t
    ...
    with ADOCommand1 do begin       //обновление поля fifth_c
    ...
    with ADOCommand1 do begin       //обновление поля fifth_p
    ...
    with ADOCommand1 do begin       //обновление поля fifth_a
    ...
    with ADOCommand1 do begin       //обновление поля fifth_m
    ...
    6 : begin
    with ADOCommand1 do begin       //обновление поля sixth_d
    ...
    with ADOCommand1 do begin       //обновление поля sixth_t
    ...
    with ADOCommand1 do begin       //обновление поля sixth_c
    ...
    with ADOCommand1 do begin       //обновление поля sixth_p
    ...
    with ADOCommand1 do begin       //обновление поля sixth_a
    ...
    with ADOCommand1 do begin       //обновление поля sixth_m
    ...
    7 : begin
    with ADOCommand1 do begin       //обновление поля seventh_d
    ...
    with ADOCommand1 do begin       //обновление поля seventh_t
    ...
    with ADOCommand1 do begin       //обновление поля seventh_c
    ...
    with ADOCommand1 do begin       //обновление поля seventh_p
    ...
    with ADOCommand1 do begin       //обновление поля seventh_a
    ...
    with ADOCommand1 do begin       //обновление поля seventh_m
    ...
    8 : begin
    with ADOCommand1 do begin       //обновление поля eigth_d
    ...
    with ADOCommand1 do begin       //обновление поля eigth_t
    ...
    with ADOCommand1 do begin       //обновление поля eigth_c
    ...
    with ADOCommand1 do begin       //обновление поля eigth_p
    ...
    with ADOCommand1 do begin       //обновление поля eigth_a
    ...
    with ADOCommand1 do begin       //обновление поля eigth_m
    ...
    9 : begin
    with ADOCommand1 do begin       //обновление поля nineth_d
    ...
    with ADOCommand1 do begin       //обновление поля nineth_t
    ...
    with ADOCommand1 do begin       //обновление поля nineth_c
    ...
    with ADOCommand1 do begin       //обновление поля nineth_p
    ...
    with ADOCommand1 do begin       //обновление поля nineth_a
    ...
    with ADOCommand1 do begin       //обновление поля nineth_m
    ...
    10 : begin
    with ADOCommand1 do begin       //обновление поля tenth_d
    ...
    with ADOCommand1 do begin       //обновление поля tenth_t
    ...
    with ADOCommand1 do begin       //обновление поля tenth_c
    ...
    with ADOCommand1 do begin       //обновление поля tenth_p
    ...
    with ADOCommand1 do begin       //обновление поля tenth_a
    ...
    with ADOCommand1 do begin       //обновление поля tenth_m
    ...
    11 : begin
    with ADOCommand1 do begin       //обновление поля eleventh_d
    ...
    with ADOCommand1 do begin       //обновление поля eleventh_t
    ...
    with ADOCommand1 do begin       //обновление поля eleventh_c
    ...
    with ADOCommand1 do begin       //обновление поля eleventh_p
    ...
    with ADOCommand1 do begin       //обновление поля eleventh_a
    ...
    with ADOCommand1 do begin       //обновление поля eleventh_m
    ...
    with ADOCommand1 do begin       //обновление поля num_f
...
begin
    with ADOCommand1 do begin       //обновление поля kode
    ...
    with ADOCommand1 do begin       //обновление поля adres
    ...
    with ADOCommand1 do begin       //обновление поля car
    ...
    with ADOCommand1 do begin       //обновление поля zakaz
    ...
    with ADOCommand1 do begin       //обновление поля place_end
    ...
    with ADOCommand1 do begin       //обновление поля priznak
...
{
//1. Изменения в таблице звонки
  if DBGrid1.Fields[0].AsInteger>0 then begin //если звонок принят автоматически
     with ADOCommand1 do begin       //находим запись в таблице звонков
  CommandText := 'select * from zvonki where num=:PNUM'; //выбираем запись для этого абонента
  CommandType := cmdText;
  Parameters.ParamByName('PNUM').Value:=DBGrid1.Fields[0].AsInteger;
  ADODataSet1.Recordset := Execute;
  end;

  if (ADODataSet1.RecordCount>0)  then begin //если запись в таблице звонков есть обновляем значения ячеек

      ObnovZvonki(flag_abon);
  end
  else begin
      //значит, почистили таблицу звонки, делаем как будто принят вручную
 with ADOCommand1 do begin       //добавление записей если с машиной
  CommandText := 'INSERT INTO zvonki ' +
 ...
 //////
  ...
  ADODataSet1.Recordset := Execute;
   ...
   else begin  //если записи нет, добавляем строку
...
var X,Y : integer;
    pt : tpoint;
begin //действия на двойной клик на таблице ZAKAZI
...
procedure TForm1.DBGrid1KeyDown(Sender: TObject; var Key: Word;
 ...
 params : TStringList;
 LgPs : String;
begin  //обрабатываем нажатие кнопки на таблице
case Key of
VK_RETURN : begin
 case DBGrid1.SelectedField.FieldNo of
 12 : begin
...
{ if DBGrid1.Fields[2].AsString <> '' then begin //если абонент
 ...
 //DBGrid1.Fields[10].AsInteger:=Round(CostPerc);
   ...
   if MessageDlg('Отработать заказ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
   OtrabStoimost(NumF);
   end;
 end;
 ...
 //стоимость
 if MessageDlg('Отработать заказ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    with ADOCommand1 do begin
    ...
    params.Free;
     if (POS('"error":"none"',resp)=0) then ShowMessage('Ошибка удаления заказа');
 end;
 ...
 // DBGrid3.SetFocus;
  ...
  DBGrid1.DataSource.DataSet.Next;
    ...
    with ADOCommand1 do begin       //обновление поля место высадки
 ...
 9 : begin
 ZakazNum:=DBGrid1.Fields[0].AsInteger;
    with ADOCommand1 do begin       //обновление поля en
    CommandText := 'UPDATE zakazi SET en=:PEN ' +
    'WHERE zakaz=:PZAKAZ';
    CommandType := cmdText;
    Parameters.ParamByName('PEN').Value:=FormatDateTime('hh:mm',Now);
 ...
 8 : begin
 ZakazNum:=DBGrid1.Fields[0].AsInteger;
    with ADOCommand1 do begin       //обновление поля beg
    CommandText := 'UPDATE zakazi SET beg=:PBEG ' +
    'WHERE zakaz=:PZAKAZ';
    CommandType := cmdText;
    Parameters.ParamByName('PBEG').Value:=FormatDateTime('hh:mm',Now);
...
// ShowMessage(MyString);
 with ADOCommand1 do begin
  ...
  else ShowMessage('Такого водителя нет!');
 ...
 //-------------------------------------------------------------------------------
    ...
    with ADOCommand1 do begin       //обновление таблицы машин
...
//--------------------------------------------------------------------------------------
    ...
    else  ShowMessage('Такой машины нет в очереди!');
      end
      else begin  //если запись в таблице заказы одна
      with ADOCommand1 do begin
      CommandText := 'SELECT * from point_queues '+
      'WHERE car = :PCAR';
       CommandType := cmdText;
 ...
 2 : begin //телефон
  ...
  DBGrid1.DataSource.DataSet.Next;
    ...
    with ADOCommand1 do begin       //обновление поля telefon
   ...
   Open;
    ...
    if DBGrid1.DataSource.DataSet.RecordCount>1 then begin  //возвращаем на место фокус ячейки
 DBGrid1.DataSource.DataSet.First;
 while not DBGrid1.DataSource.DataSet.Eof do begin
  if DBGrid1.Fields[0].AsInteger=ZakazNum then break;
 DBGrid1.DataSource.DataSet.Next;
...
//1. изменения в таблице абоненты

  if ADODataSet1.Recordset.Fields[7].Value<>NULL then begin //определяем значение Balans
    ...
    Balans:=0;
    end;   //определили значение Balans

  if ADODataSet1.Recordset.Fields[8].Value<>NULL then begin //определяем значение Poezdok
    ...
    Poezdok:=0;
    end;   //определили значение Poezdok

  if ADODataSet1.Recordset.Fields[10].Value<>NULL then begin //определяем значение Priz
    ...
    Priz:=0;
    end;   //определили значение Priz

   if DBGrid1.Fields[11].AsString='призовая' then begin
   flag_priz:=1;
   ObnovAbonenty(flag_priz); //выполнили процедуру обновления таблицы абонентов
   end
   else begin
  //  ShowMessage('очередная поездка');
    ObnovAbonenty(flag_priz);
   end;
//2. Изменения в таблице звонки
  if DBGrid1.Fields[0].AsInteger>0 then begin //если звонок принят автоматически
     with ADOCommand1 do begin       //находим запись в таблице звонков
  CommandText := 'select * from zvonki where num=:PNUM'; //выбираем запись для этого абонента
  CommandType := cmdText;
...
//3 Изменения в таблице машин
 ...
 if (CarBee>0) and (ADODataSet1.Recordset.RecordCount>0) then begin  //Если есть машина, переставляем
StrGrdCel:=CarBee;

StrGrdRow:=ADODataSet1.Recordset.Fields[1].Value;
IF (StrGrdRow=1) then  PerestavCarStrGrid(0);
 end
else {ShowMessage('В очереди нет машины с таким номером!')};


//6 Удаление из таблицы заказов
with ADOCommand1 do begin
  CommandText := 'DELETE FROM zakazi WHERE ZAKAZ=:PZAKAZ';
  CommandType := cmdText;
  Parameters.ParamByName('PZAKAZ').Value:=DBGrid1.Fields[0].AsInteger;
  ...
  ShowMessage('Абонента с таким номером не существует!');
  end;
end
else begin   //Если это не абонент
...
//2 Изменения в таблице очередь машин
...
//3 Изменения в таблице водители
{
     with ADOCommand1 do begin       //находим запись в таблице звонков
  CommandText := 'select * from cars where num=:PNUM'; //выбираем запись для этого абонента
  CommandType := cmdText;
 ...
 if (ADODataSet1.RecordCount>0) then  begin
    ...
    with ADOCommand1 do begin       //обновление поля summa
...
//4 Изменения в таблице диспетчеров
  // ShowMessage('диспетчер ');
  {   with ADOCommand1 do begin       //находим запись в таблице диспетчеры
  CommandText := 'select * from disp where num=:PNUM'; //выбираем запись для этого диспетчера
  CommandType := cmdText;
  Parameters.ParamByName('PNUM').Value:=Working;
  ADODataSet1.Recordset := Execute;
  end;
 if (ADODataSet1.RecordCount>0) then  begin
  if DBGrid1.Fields[13].AsInteger = Working then begin //проверяем не поменялся ли диспетчер

  end
  else begin    //если поменялся диспетчер
    //ShowMessage('диспетчер новый');
     with ADOCommand1 do begin       //находим запись в таблице диспетчеры
  CommandText := 'select * from disp where num=:PNUM'; //выбираем запись для этого диспетчера
  CommandType := cmdText;
...
//flag_abon:=0;
   Bookmark := DBGrid1.DataSource.DataSet.Bookmark;
   with ADOQuery1 do begin  //отображение очереди машин в таблице
   SQL.Clear;
   SQL.Add('select zakaz,telefon,kode,dat,tim,adres,car,beg,en,place_end,cost,priznak,predvar,working,uvedomlen,vip from zakazi order by zakaz');
   Open;
...
begin  //нажатие на меню очередная
//PriznPoezd:='очередная';
    with ADOCommand1 do begin       //обновление поля priznak
...
begin
//PriznPoezd:='отложенная';
    with ADOCommand1 do begin       //обновление поля priznak
    CommandText := 'UPDATE zakazi SET priznak=:PPRIZNAK ' +
    'WHERE zakaz=:PZAKAZ';
    CommandType := cmdText;
    Parameters.ParamByName('PPRIZNAK').Value:='отложенная';
    Parameters.ParamByName('PZAKAZ').Value:=ZakazNum;
    Execute;
    end;
   with ADOQuery1 do begin  //отображение очереди машин в таблице
...
begin   //пункт меню В очередь
Application.CreateForm(TForm3, Form3);
Form3.Caption:='В очередь';
...
if Length(Edit1.Text)>0 then begin
 ...
 // Parameters.ParamByName('PSTATE').Value:=1;
  Execute;
   ...
   with Form1.ADOQuery4 do begin  //отображение  машин в таблице
   SQL.Clear;
...
begin  //пункт меню Из очереди
  ...
  Execute;
   ...
   with ADOQuery4 do begin  //отображение  машин в таблице
   SQL.Clear;
   SQL.Add('select * from cars order by num');
   Open;
   end;
...
begin  //добавляем машину в таблицу машин
Application.CreateForm(TForm3, Form3);
Form3.Caption:='Добавить машину';
Form3.Button1.Visible:=false;
Form3.ShowModal;
Form3.Free;
  {   with ADOCommand1 do begin       //находим запись в таблице машины
  CommandText := 'select * from cars order by num'; //выбираем запись для этой машины
  CommandType := cmdText;
  ADODataSet1.Recordset := Execute;
  end;
 if (ADODataSet1.RecordCount>0) then  begin
 ...
 NumCar:=ADODataSet1.Recordset.Fields[0].Value;
  ...
  Execute;
   ...
   with ADOCommand1 do begin       //находим запись в таблице машины
  CommandText := 'select * from cars where num=:PNUM order by num'; //выбираем запись для этой машины
  CommandType := cmdText;
  Parameters.ParamByName('PNUM').Value:=StrToInt(Edit1.Text);
  ADODataSet1.Recordset := Execute;
  end;
  if (ADODataSet1.RecordCount=0) then  begin
   //ShowMessage('Такой машины нет');
    with ADOCommand1 do begin
  CommandText := 'INSERT INTO cars (num,summa,week) VALUES(:PNUM,0,0)';
  CommandType := cmdText;
  Parameters.ParamByName('PNUM').Value:=StrToInt(Edit1.Text);
  ...
  else ShowMessage('Такая машина уже есть!');
with ADOCommand1 do begin       //добавление записей если абонент
  CommandText := 'INSERT INTO zvonki ' +
    '(DAT,TIM,ADRES) ' +
  ...
  Parameters.ParamByName('PDAT').Value:=FormatDateTime('yyyy-mm-dd',Now);
  Parameters.ParamByName('PTIM').Value:=FormatDateTime('hh:mm',Now);
  Parameters.ParamByName('PADRES').Value:='Оператор '+IntToStr(Working)+' Доб Маш '+Edit1.Text;
  Execute;
  end;
   with ADOQuery4 do begin  //отображение  машин в таблице
   SQL.Clear;
   ...
   with ADOQuery2 do begin  //отображение  машин в таблице
   SQL.Clear;
   SQL.Add('select num,telefon,kode,dat,tim,adres,car,beg,en,cost,priznak,zakaz from zvonki order by num');
   Open;
   end;
...
begin  //убираем машину из таблицы машин
with ADOCommand1 do begin       //добавление записей если абонент
  ...
  Parameters.ParamByName('PDAT').Value:=FormatDateTime('yyyy-mm-dd',Now);
  Parameters.ParamByName('PTIM').Value:=FormatDateTime('hh:mm',Now);
  Parameters.ParamByName('PADRES').Value:='Оператор '+IntToStr(Working)+' Удал Маш '+DBGrid4.Fields[0].AsString;
  Execute;
  end;
with ADOCommand1 do begin       //добавление записей если абонент
  ...
  Execute;
   ...
   with ADOQuery2 do begin  //отображение  звонки
   SQL.Clear;
   SQL.Add('select num,telefon,kode,dat,tim,adres,car,beg,en,cost,priznak,zakaz from zvonki order by num');
   Open;
   end;
...
begin  //неделю закончить
     with ADOCommand1 do begin       //находим запись в таблице очередь машин
  CommandText := 'select * from cars where num=:PCAR'; //выбираем запись для этого диспетчера
  CommandType := cmdText;
...
with ADOCommand1 do begin       //добавление записей если абонент
  ...
  Parameters.ParamByName('PDAT').Value:=FormatDateTime('yyyy-mm-dd',Now);
  Parameters.ParamByName('PTIM').Value:=FormatDateTime('hh:mm',Now);
  Parameters.ParamByName('PADRES').Value:='Опер '+IntToStr(Working)+' Обнол Нед Маш '+DBGrid4.Fields[0].AsString;
  Execute;
  end;
   with ADOCommand1 do begin
 ...
 else ShowMessage('Такой машины нет!');
   with ADOQuery4 do begin  //отображение  машин в таблице
   SQL.Clear;
   SQL.Add('select * from cars order by num');
   Open;
...
begin  // меню Добавить диспетчера
Application.CreateForm(TForm3, Form3);
Form3.Caption:='Добавить диспетчера';
Form3.LabeledEdit1.EditLabel.Caption:='Введи номер диспетчера';
Form3.Button2.Visible:=false;
Form3.ShowModal;
Form3.Free;
//ShowMessage(Edit1.Text);
...
//ShowMessage(Edit1.Text);
   with ADOCommand1 do begin       //находим запись в таблице машины
  CommandText := 'select * from disp where num=:PNUM '; //выбираем запись для этой машины
  CommandType := cmdText;
  Parameters.ParamByName('PNUM').Value:=StrToInt(Edit1.Text);
  ADODataSet1.Recordset := Execute;
  end;
  if (ADODataSet1.RecordCount=0) then  begin
  //ShowMessage('Есть такой диспетчер!');
  with ADOCommand1 do begin       //добавление записей если абонент
  ...
  else ShowMessage('Есть такой диспетчер!');
end
else ShowMessage('Не ввели диспетчера!');
   with ADOQuery5 do begin  //отображение  диспетчеров в таблице
   SQL.Clear;
...
begin   //меню удалить диспетчера
 ...
 if (ADODataSet1.RecordCount>0) then  begin
  with ADOCommand1 do begin       //удаление записи
 ...
 else ShowMessage('Диспетчер уже удален!');
   with ADOQuery5 do begin  //отображение  диспетчеров в таблице
   SQL.Clear;
   SQL.Add('select * from disp order by num');
   Open;
...
begin   //меню принять смену
 ...
 Working:=DBGrid5.Fields[0].AsInteger;
  with ADOCommand1 do begin       // отметка в таблице звонков
  CommandText := 'INSERT INTO zvonki ' +'(DAT,TIM,ADRES) ' +
    'VALUES (:PDAT,:PTIM,:PADRES)';
  CommandType := cmdText;
  Parameters.ParamByName('PDAT').Value:=FormatDateTime('yyyy-mm-dd',Now);
  Parameters.ParamByName('PTIM').Value:=FormatDateTime('hh:mm',Now);
  Parameters.ParamByName('PADRES').Value:='Опер '+DBGrid5.Fields[0].AsString+' принял смену';
  Execute;
  end;
   with ADOCommand1 do begin
 ...
 else ShowMessage('Один диспетчер уже на смене!');
 end
 else ShowMessage('Такого диспетчера не существует!!');
   with ADOQuery5 do begin  //отображение  диспетчеров в таблице
   SQL.Clear;
   SQL.Add('select * from disp order by num');
   Open;
   end;
   with ADOQuery2 do begin  //отображение  звонков в таблице
   SQL.Clear;
   SQL.Add('select num,telefon,kode,dat,tim,adres,car,beg,en,cost,priznak,zakaz from zvonki order by num');
   Open;
   end;
...
begin     //меню сдать смену
 ...
 Working:=0;
  ...
  Parameters.ParamByName('PDAT').Value:=FormatDateTime('yyyy-mm-dd',Now);
  Parameters.ParamByName('PTIM').Value:=FormatDateTime('hh:mm',Now);
  Parameters.ParamByName('PADRES').Value:='Опер '+DBGrid5.Fields[0].AsString+' сдал смену';
  Execute;
  end;
   with ADOCommand1 do begin
  ...
  else ShowMessage('Этот диспетчер не на смене!');
 end
 else ShowMessage('Такого диспетчера не существует!!');
   with ADOQuery5 do begin  //отображение  диспетчеров в таблице
   SQL.Clear;
...
begin  //меню завершить неделю диспетчера
  with ADOCommand1 do begin       //находим запись в таблице диспетчеры
  CommandText := 'select * from disp where num=:PNUM';
  CommandType := cmdText;
  Parameters.ParamByName('PNUM').Value:=DBGrid5.Fields[0].AsInteger;
 ...
 if (ADODataSet1.RecordCount>0) then  begin
  ...
  Parameters.ParamByName('PDAT').Value:=FormatDateTime('yyyy-mm-dd',Now);
  Parameters.ParamByName('PTIM').Value:=FormatDateTime('hh:mm',Now);
  Parameters.ParamByName('PADRES').Value:='Опер '+IntToStr(Working)+' Обнол Нед Оп-ру '+DBGrid5.Fields[0].AsString;
  Execute;
  end;
   with ADOCommand1 do begin
...
begin  //меню Работа с абонентами
...
begin  //переключаемся между закладками
//ShowMessage(IntToStr(PageControl1.ActivePageIndex));
case PageControl1.ActivePageIndex of
0 : begin //обновление заказов и очереди машин
   with ADOQuery1 do begin  //отображение очереди машин в таблице
   SQL.Clear;
   SQL.Add('select zakaz,telefon,kode,dat,tim,adres,car,beg,en,place_end,cost,priznak,predvar,working,uvedomlen,vip from zakazi order by zakaz');
   Open;
...
1 : begin  //обновление таблицы звонков
   with ADOQuery2 do begin  //отображение звонков
   SQL.Clear;
   ...
   DBGrid2.DataSource.DataSet.MoveBy(DBGrid2.DataSource.DataSet.RecordCount-1); //передвигаем указательв конец
   end;
end;
2: begin
//обновление поля day в таблице disp
     with ADOCommand1 do begin
  CommandText := 'select * from zvonki where adres like '+ '''%'+'принял смену''';
  CommandType := cmdText;
  ADODataSet1.Recordset := Execute;
  end;
 if (ADODataSet1.RecordCount>0) then  begin
 ...
 // CommandText := 'select sum(cost) from zvonki where dat>='''+FormatDateTime('yyyy-mm-dd',ADODataSet1.Recordset.Fields[3].Value)+''';
  ...
  Log.Lines.Add('update disp set day=(select sum(cost) from zvonki where dat+tim>='''+ FormatDateTime('yyyy-mm-dd',ADODataSet1.Recordset.Fields[3].Value)+
   ...
   with ADOQuery4 do begin  //отображение машин
   SQL.Clear;
   SQL.Add('select * from cars order by num');
   Open;
   end;
   with ADOQuery5 do begin  //отображение диспетчеров
   SQL.Clear;
   SQL.Add('select * from disp order by num');
   Open;
   end; 
...
begin  //запускаем командный файл на создание отчета
 ShellExecute(Handle,nil,'report.bat',nil,nil,SW_RESTORE);
 ShowMessage('Файл отчета C:/report.txt');
end;
...
begin    //обработка предварительных заказов
 with ADOCommand1 do begin       //есть ли предварительный заказ
  CommandText := 'SELECT * FROM zakazi ' +
...
if ADODataSet1.RecordCount>0 then begin  //если есть предварительный заказ
ADODataSet1.Recordset.MoveFirst;
//в цикле проверяем если подходит время -  короткий сигнал
Present:= Now;
...
//if (ADODataSet1.Recordset.Fields[13].Value = 'YES')  then begin    end;
  ...
  //ShowMessage('Есть предварительный короткий');
...
begin  //автомобиль на Той стороне
with ADOCommand1 do begin       //обновлени поля state
...
begin //refresh stringgrid1
 CntCol:=7;
 ColGrd:=0;
 RowGrd:=0;  //инициализируем массив
...
//находим наибольшее количество машин в строках
with ADOCommand1 do begin
   ...
   ADODataSet1.First;
  LVar:=ADODataSet1.Recordset.Fields[0].Value; //преобраз из OLEVariant в INT
  if StrToInt(VarToStr(LVar)) > CntCol then CntCol:= StrToInt(VarToStr(LVar));

//выбираем машины из строки
with ADOCommand1 do begin
...
//--------заполняем StringGrid значениями из массива-------------
...
ObnovStringGrig(1);
  ...
  else ShowMessage('В очереди есть такая машина!');
...
if (StGrClDbl<>0) then begin
     ...
     if (StGrRwDbl<>StrGrdRow) then begin //если не та же строка
   with ADOCommand1 do begin
   ADOCommand1.CommandText := 'select COUNT(col) from cqueue where row=:PNEWROW';
   ADOCommand1.CommandType := cmdText;
   Parameters.ParamByName('PNEWROW').Value:=StrGrdRow;
...
idHTTP1.Get(RemoteHost+'/api/' + LogPswSrw + '/reforders');
    ...
    ShowMessage('Такая машина есть на этой стоянке!');
    end;
end;
StringGrid1.Cursor:=crDefault;
...
var NewCol : integer;
    NewRow : integer;
    LVar : OleVariant;
begin  //Прoцедура перестановки автомобиля в StringGrid1
   with ADOCommand1 do begin       //находим номер в таблице последней машины
  CommandText := 'select car,row,col from cqueue where car=:PCAR';
  CommandType := cmdText;
  Parameters.ParamByName('PCAR').Value:=StrGrdCel;
...
begin  //Позвонить на номер
// if SipPhoneSt = 0 then begin
 if MessageDlg('Позвонить на номер ' + NumTelefClick + ' ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
 case RadioGroup1.ItemIndex of
...
begin  //Проиграть файл
 if MessageDlg('Уведомить номер ' + NumTelefClick + ' ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
 case RadioGroup1.ItemIndex of
...
DatBee : string;
TimBee : String;
begin //отрабатываем стоимость Пчелкам
   with ADOCommand1 do begin
...
//  ShowMessage('Отрабатываем заказ');
//1. Изменения в таблице звонки
...
ObnovZvonkiBee(ZakBee,CarBee,TelBee,KodBee,AdrBee,DatBee,TimBee);
 ...
 //ShowMessage('ПЕРЕСТАВЛЯЕМ МАШИНЫ!');
  with ADOCommand1 do begin       //находим номер в таблице последней машины
  CommandText := 'select car,row,col from cqueue where car=:PCAR';
  CommandType := cmdText;
  Parameters.ParamByName('PCAR').Value:=CarBee;
...
with ADOCommand1 do begin
 ...
 ShowMessage('Такого заказа нет в таблице!')


end;
procedure TForm1.N50Click(Sender: TObject);
begin  //машину на Парковый
...
procedure TForm1.StringGrid1KeyDown(Sender: TObject; var Key: Word;
  ...
  params : TStringList;
  resp : string;
begin   //Обрабатываем нажатие кнопки в таблице
case Key of
VK_RETURN : begin
if (StrGrdStr<>'') then begin //если не пустая ячейка
   with ADOCommand1 do begin
   ADOCommand1.CommandText := 'select * from point_queues where car=:PCAR';
   ADOCommand1.CommandType := cmdText;
   ...
   ADODataSet1.First;
   case StrGrdRow of
   0 : StaySpeak:=' на линию?';
   1  : StaySpeak:=' на заказ?';
   2 : StaySpeak:=' на ЧЕРЕМУШКИ?';
   3 : StaySpeak:=' на ЭЛЕВАТОР?';
   4 : StaySpeak:=' на ГЛОБУС?';
   5 : StaySpeak:=' на РЫНОК?';
   6 : StaySpeak:=' на МАГНОЛИЮ?';
   7 : StaySpeak:=' на ПЕНТАГОН?';
   8 : StaySpeak:=' на ВОЕННЫЙ?';
   9 : StaySpeak:=' на ТА СТОРОНА?';
   10 : StaySpeak:=' на ПАРКОВЫЙ?';
   else StaySpeak:=' на пробел?';
   end;
    if MessageDlg('Переставить машину ' + StrGrdStr + StaySpeak ,mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
    //начинаем обновление таблицы
    with ADOCommand1 do begin
    ADOCommand1.CommandText := 'select login,   password, car from users where car=:PCAR';
    ADOCommand1.CommandType := cmdText;
    Parameters.ParamByName('PCAR').Value:=StrGrdStr;
    ...
    //ShowMessage(LgPs);
//Шлем запрос на перестановку машин
  if MessageDlg('Отработать заказ для машины ' + StrGrdStr + '?' ,mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
  params:=TStringList.Create;
...
//  params.Add('delzak=0');
  ...
  params.Free;
  if (POS('"error":"none"',resp)=0) then ShowMessage('Ошибка перестановки');
   params:=TStringList.Create;
   resp:=idHTTP1.Post(RemoteHost+'/api/' + LgPs + '/orderdestroy',params);
   params.Free;
   if (POS('"error":"none"',resp)=0) then ShowMessage('Ошибка удаления заказа');
  ...
  idHTTP1.Get(RemoteHost+'/api/' + LogPswSrw + '/reforders');
   ...
   ShowMessage('Такой машины нет в таблице!');
   ObnovStringGrig(1);
   end;
end;
...
begin //переносим машину в другую строку
...
//  Parameters.ParamByName('PZAKAZ').Value:=NumZak;
  ...
  Execute;
   ...
   with ADOQuery1 do begin  //отображение таблицы заказов
   SQL.Clear;
   SQL.Add('select zakaz,telefon,kode,dat,tim,adres,car,beg,en,place_end,cost,priznak,predvar,working,uvedomlen,vip from zakazi order by zakaz');
   Open;
   end;
...
Form7.Caption:='Работа с таблицей заказов';
Form7.ShowModal;
Form7.Free; }
...
CQCol :=1;
 ...
 //   ShowMessage(IntToStr(CQue[CQRow,CQCol])+IntToStr(CQue[CQRow,CQCol+1])+IntToStr(CQue[CQRow,CQCol+2])+IntToStr(CQue[CQRow,CQCol+3]));
    ...
    while ColCque-i>=0 do begin //проверяем есть ли автомобили с одинаковым столбцом
   // ShowMessage('ColCque='+IntToStr(ColCque));
     ...
     // ShowMessage('ACHTUNG! '+IntToStr(CQue[CurRowDN,4])+IntToStr(CQue[CurRowDN,1])+IntToStr(CQue[CurRowDN,2])+IntToStr(CQue[CurRowDN,3]));
      ...
      ShowMessage('автомобиль в дырке: '+IntToStr(ErrCar));
      break;
      end;
      end;
     i:=i+1;
     ...
     if (FlagDyrka=1) then begin  //ищем в каком месте дырка
      for i:=2 to ColCque do begin
       if (CQue[i,2]-CQue[i-1,2]<>1) then begin
       //ShowMessage('дырка в строке '+IntToStr(i));
       break;
       end;
      end;
     end;
    if (FlagDyrka=1) then begin  //передвигаем автомобиль
      with ADOCommand1 do begin
      CommandText := 'update cqueue set col=:PCOL where car=:PCAR and row=:PROW';
      CommandType := cmdText;
   ...
   else ShowMessage('Строка пустая');
end;
...
head:= idHTTP1.ResponseText;
Memo2.Lines.Add(head);
Memo2.Lines.Add('На SMS-счете:  '+resp+' руб');
end;
...
resp, head : string;
begin
 if MessageDlg('Послать SMS на номер ' + NumTelefClick + ' ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
   if (Length(NumTelefClick) = 10) and (NumTelefClick[1] = '9') then begin
 resp := idHTTP1.Get('http://smsc.ru/sys/send.php?' + 'login=taxipchelka' +
 '&psw=gjuht,bwrbq' + '&phones=+7' + NumTelefClick + '&mes=Заказанная машина прибыла. Такси Пчелка желает Вам приятной поездки!'
  +  '&sender=TAXI');
head:= idHTTP1.ResponseText;
 StatusBar1.Panels[1].Text:=head;
   with ADOCommand1 do begin
  ...
  Execute;
   ...
   else //ShowMessage('Неправильный номер телефона!');
   UvedGorAbon();
   end;
...
begin
  ...
  ADODataSet1.Recordset := Execute;
   ...
   CarTelef:=ADODataSet1.Recordset.Fields[1].Value;
    if MessageDlg('Послать SMS водителю '+ CarClick +' на номер ' + CarTelef + ' ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
  ...
  // end
   ...
   else ShowMessage('У водителя нет телефона!');
    end
   else  ShowMessage('Нет такого водителя!');
end;
...
{head:= idHTTP1.ResponseText;
 ...
 {  with ADOCommand1 do begin
  ...
  // ClientSocket1.Socket.SendText(Priority+'REFRESH');
  Str:='UVE;' + IntToStr(NumZakClick) + ';' + NumTelefClick + ';' + 'Заказанная машина прибыла. Такси Пчелка желает Вам приятной поездки!';
...
procedure TForm1.ClientSocket3Connecting(Sender: TObject;
  Socket: TCustomWinSocket);
begin
StatusBar1.Panels[1].Text:='SIP подсоединяется...';
end;

procedure TForm1.ClientSocket3Connect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
StatusBar1.Panels[1].Text:='SIP подсоединен';
end;
...
begin
//Читаем сообщения от SIP-телефона
s:=Socket.ReceiveText;
 if (s[1]+s[2]+s[3]='DAT') then begin    //значит приходят данные
 Delete(s,1,3);
 ...
 CallsProcessing(CallingNumb, 'asterisk'); //заносим в Zvonki
 ...
 if (s[1]+s[2]+s[3]='NAS') then begin    //значит приходят Not Assigned Channel
  //ShowMessage(s);
   case ChanAssign of
   1: begin Label13.Color:=clActiveBorder; Label13.Caption:='ОТБОЙ' end;
   2: begin Label14.Color:=clActiveBorder; Label14.Caption:='ОТБОЙ' end;
   3: begin Label15.Color:=clActiveBorder; Label15.Caption:='ОТБОЙ' end;
   4: begin Label16.Color:=clActiveBorder; Label16.Caption:='ОТБОЙ' end;
   5: begin Label17.Color:=clActiveBorder; Label17.Caption:='ОТБОЙ' end;
   6: begin Label20.Color:=clActiveBorder; Label20.Caption:='ОТБОЙ' end;
   end;
 LabeledEdit1.Text:='';
 LabeledEdit1.EditLabel.Caption:='Телефон';
 LabeledEdit2.Text:='';
